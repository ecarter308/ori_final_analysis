...
st_write(fishnet_polygon, "C:/Users/Eliza.Carter/Documents/Projects/California/ORI/data/source_data/ori_ais_grid_intersect.shp")
...
######
# write info and time to a log file
      log_line <- paste(format(Sys.time(), format = "%F %R %Z"), fname, basename(oname),
                        paste("year", year, sep="= "),
                        paste("study_area", study_area_name, sep="= "),
                        paste("month", month, sep="= "),
                        paste("res_m", output_resolution, sep="= "),
                        paste("in_file_dsn", basename(dsn_lines), sep="= "),
                        paste("in_file_summ", basename(summ_fp), sep="= "),
                        paste("in_dsn_layers", paste(layers$layer_name, collapse = " & "), sep="= "), sep = ", ")
      write(log_line, file=paste0(out_dir, "log_file.txt"), append=TRUE)
